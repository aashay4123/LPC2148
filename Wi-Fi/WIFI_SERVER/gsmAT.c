...
unsigned char receive_char(void);    //Receive a character 
...
//##########################################	  
	  ...
	  soft_delay_ms(500);
	   ...
	   send_char(0x0a);
		 ...
		 soft_delay_ms(1000);
		 send_string("AT+CWJAP=\"TP\",\"12345678\"");    //AT command to join network
		 soft_delay_ms(10);
		 send_char(0x0d);
		 soft_delay_ms(10);
		 send_char(0x0a);
	   ...
	   //Sending AT command for get IP address
     LCD_Cmd(0x01);
		 gotoloc(1,1);
		 LCD_WriteString("IP address");
		 soft_delay_ms(1000);
		 ...
		 //Sending AT command for configuring module as Server
		 LCD_Cmd(0x01);
		 soft_delay_ms(100);
		 gotoloc(1,1);
		 LCD_WriteString("Sending");
		 gotoloc(2,1);
		 LCD_WriteString("AT+CIPSERVER");
		 soft_delay_ms(1000);
		 send_string("AT+CIPSERVER=1,1336");      //AT command for setting module as server
		 soft_delay_ms(10);
		 send_char(0x0d);
		 soft_delay_ms(10);
		 send_char(0x0a);
...
//Function to send character using UART1
